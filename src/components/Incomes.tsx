...
import { AppState, Income, IncomeCategory } from '../types';
...
export default function Incomes({ data, onUpdate }: IncomesProps) {
  ...
  const filteredIncomes = data.incomes.filter(i => {
    ...
    const matchesDate = isWithinInterval(new Date(i.date), { 
                ...
                <tr key={income.id} className="hover:bg-white/[0.02] transition-colors group">
                  <td className="px-4 md:px-6 py-3 md:py-4">
                    ...
                    <div className="md:hidden flex items-center gap-2 mt-0.5 text-[15px] text-white/40">
                      ...
                      <span>•</span>
                      <div className="flex items-center gap-1">
                        <span>{format(new Date(income.date), 'dd MMM', { locale: es })}</span>
